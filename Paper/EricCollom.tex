...
In order to clarify, let $\alpha$ and $\beta$ be sections of code of two programs \texttt{A} and \texttt{B}, respectively, on which crossover is being applied. Stack depth, variable type, and control flow were the semantic constraint for which the checks were done to assure good crossover. The following is how these checks were done.\par
...
Evolving entire programs at the instruction-level is possible at the source code level. No assumptions about the program being evolved is needed, and it is easier to deal with the semantic constraints. Orlov and Sipper, and Schulte, et al.,~\cite{FINCH:2011,Assembly:2010} showed that evolving instruction-level code is just as feasible as source code and at times better, such as for debugging incorrect type declarations. In conclusion, evolving instruction-level code is feasible and exciting for the field of EC. It opens up many possibilities that were once unavailable.
